      ...
      <div className="row">
        <div className={selectedOrder ? 'col-7' : 'col-12'}>
          {list.map((order) => {
            ...
            return (
              <div key={order.id} className="card mb-3 shadow-sm" style={{ cursor: 'pointer' }}>
                  ...
                  <div className="d-flex justify-content-between align-items-start">
                    <div className="flex-grow-1" onClick={() => setSelectedOrder(order)}>
                      ...
                      <div className="row small text-secondary mb-2">
                        <div className="col-md-3">📦 {productsCount} Продукта</div>
                        <div className="col-md-4">📅 {formatDate(order.createdAt, 'short')}</div>
                        <div className="col-md-5">
                          💵 {totalUSD.toFixed(2)} $ | {totalUAH.toFixed(2)} UAH
                        </div>
                      </div>
                      <div className="text-secondary small">{formatDate(order.createdAt, 'long')}</div>
                    </div>
                    <button
